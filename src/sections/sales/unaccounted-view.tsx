...
const CompactRegisterClosingUI: React.FC = () => {
  ...
  // 未会計予約のステート
  const [unaccountedReservations, setUnaccountedReservations] = useState<
    Reservation[]
  >([]);
  ...
  const fetchLatestClosingDate = async () => {
    ...
    try {
      ...
      if (closing_date) {
        // 日本時間でフォーマット
        const formattedDateTime = dayjs(closing_date)
          .tz("Asia/Tokyo")
                  ...
                  {PAYMENT_METHODS.map((method) => (
                    <TableRow key={method}>
                      ...
                      <TableCell className="text-right py-1">
                        {totalByPaymentMethod[method]?.toLocaleString() || "0"}{" "}
                        円
                      </TableCell>
                    </TableRow>
                  ))}
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center">
                    <Input
                      ...
                      value={preparedCash}
                      onChange={(e) =>
                        setPreparedCash(parseInt(e.target.value) || 0)
                      }
                    />
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center">
                    <Input
                      ...
                      value={cashOut}
                      onChange={(e) =>
                        setCashOut(parseInt(e.target.value) || 0)
                      }
                    />
                    <span
                      className={`ml-1 ${
                        cashOut < 0 ? "text-red-500" : "text-green-500"
                      }`}
                    >
                      {cashOut < 0 ? "-" : ""}
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold">
                    {(
                      (totalByPaymentMethod["現金"] || 0) + preparedCash
                    ).toLocaleString()}{" "}
                    円
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center mt-1">
                    <Input
                      ...
                      value={actualCash}
                      onChange={(e) =>
                        setActualCash(parseInt(e.target.value) || 0)
                      }
                    />
                    <span className="ml-1">円</span>
                  </div>
              ...
              onChange={(e) => {
                ...
                if (selectedDate) {
                  ...
                  const selectedDateTime = dayjs(
                    `${selectedDate} ${now.format("HH:mm:ss")}`,
                    "YYYY-MM-DD HH:mm:ss"
                  )
                    .tz("Asia/Tokyo")
                    .utc()
                  ...
                  <TableRow key={reservation.id}>
                    ...
                    <TableCell>
                      {reservation.customer?.name ||
                        reservation.customer?.name_kana ||
              ...
              <TableBody>
                {accountingList.map((item) => (
                  <TableRow key={item.id}>
                    {/* 来店日時を日時含む形式で表示 */}
                    <TableCell>
                      {dayjs(item.reservations.start_time)
                        .tz("Asia/Tokyo")
                        .format("YYYY-MM-DD HH:mm:ss")}
                    </TableCell>
