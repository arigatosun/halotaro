...
const CalendarView = forwardRef<FullCalendar, CalendarViewProps>(
  (
    ...
    const earliestOpenTime = moment
      .min(businessHours.map((bh) => moment(bh.open_time, 'HH:mm:ss')))
      .format('HH:mm:ss');
    const latestCloseTime = moment
      .max(businessHours.map((bh) => moment(bh.close_time, 'HH:mm:ss')))
      .format('HH:mm:ss');
    ...
    .filter(
      ...
      .map((reservation) => ({
        id: reservation.id,
        resourceId: reservation.staff_id.toString(), // 文字列に変換
    ...
    return (
      ...
      >
        <StyledFullCalendar
          ...
          initialDate={currentDate.format('YYYY-MM-DD')} // 追加
          editable={true}
          selectable={true}
          select={onDateSelect}
