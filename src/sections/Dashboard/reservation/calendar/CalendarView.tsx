...
const CalendarView = forwardRef<FullCalendar, CalendarViewProps>(
  (
    ...
    const earliestOpenTime = moment
      .min(businessHours.filter(bh => !bh.is_holiday).map((bh) => moment(bh.open_time, 'HH:mm:ss')))
      .format('HH:mm:ss');
    const latestCloseTime = moment
      .max(businessHours.filter(bh => !bh.is_holiday).map((bh) => moment(bh.close_time, 'HH:mm:ss')))
      .format('HH:mm:ss');
    ...
    return (
      ...
      >
        <StyledFullCalendar
          ...
          businessHours={businessHours.filter(bh => !bh.is_holiday).map(bh => ({
