...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 予約データの整形
    let reservations = (customerData.reservations || []).map((reservation: any) => {
      ...
      if (reservation.accounting_information) {
        ...
        if (Array.isArray(reservation.accounting_information.items)) {
          ...
          accountingItems.forEach((item: any) => {
            const category = item.category; // '施術' または '店販'
        ...
        // accounting_informationがない場合、金額を0に設定
        amount = 0;
        technicalAmount = 0;
        productAmount = 0;
      }
      ...
      return {
        ...
        status: mappedStatus,
        menu: reservation.menu_item?.name || "未指定",
        amount: amount,
    ...
    const cancelledReservations = reservations.filter((r: any) =>
      ["お客様キャンセル", "サロンキャンセル", "当日キャンセル", "無断キャンセル"].includes(r.status)
    );
