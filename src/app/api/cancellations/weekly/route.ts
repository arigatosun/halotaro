// app/api/cancellations/weekly/route.ts
...
export async function GET(req: NextRequest) {
  try {
    ...
    const weeks = Array.from({ length: 4 }, (_, i) => today.subtract(i, "week"));
    ...
    const sortedWeeklyData = weeklyData.sort((a, b) => {
      const dateA = dayjs(a.date.split(" - ")[0], "M/D");
      const dateB = dayjs(b.date.split(" - ")[0], "M/D");
      return dateA.isAfter(dateB) ? 1 : -1;
    });

    return NextResponse.json(sortedWeeklyData, { status: 200 });
  } catch (error: any) {
    console.error("週別キャンセルデータ取得エラー:", error);
