...
import { ProcessedCoupon } from "@/types/coupon";
...
export async function saveCoupons(
  ...
  const existingCouponMap = new Map(
    existingCoupons?.map((coupon) => [coupon.coupon_id, coupon.id])
  );
  ...
  const couponsToDeactivate = existingCoupons?.filter(
    (coupon) => !syncedCouponIds.has(coupon.coupon_id)
  );
  ...
  if (couponsToDeactivate && couponsToDeactivate.length > 0) {
    const { error: deactivateError } = await supabase
      ...
      .in(
        "id",
        couponsToDeactivate.map((coupon) => coupon.id)
      );

    if (deactivateError) {
      console.error("Error deactivating coupons:", deactivateError);
      throw new Error(
        `Failed to deactivate coupons: ${deactivateError.message}`
      );
    }
  }
  ...
  console.log(
    `Saved ${couponUpdates.length} updated coupons and ${couponInserts.length} new coupons`
  );
