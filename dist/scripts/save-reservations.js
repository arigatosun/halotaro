...
exports.saveReservations = saveReservations;
const supabaseClient_1 = require("../src/lib/supabaseClient");
const reservaitonProcessor_1 = require("../src/utils/reservaitonProcessor");
async function saveReservations(rawReservations, haloTaroUserId, salonboardUserId, startDate, endDate, dataHash, lastReservationId) {
    if (rawReservations.length === 0) {
        console.log("No new reservations to save. Skipping database operation.");
        return;
    }
    const { data: existingLog } = await supabaseClient_1.supabase
    ...
    const processedReservations = await Promise.all(rawReservations.map((raw) => (0, reservaitonProcessor_1.processReservation)(raw, haloTaroUserId)));
    console.log("Processed Reservations:", JSON.stringify(processedReservations, null, 2));
    const { data, error } = await supabaseClient_1.supabase.rpc("upsert_reservations_and_log", {
