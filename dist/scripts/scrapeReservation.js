...
async function scrapeReservations(page, startDate, endDate, lastSyncTime, lastReservationId) {
    ...
    while (shouldContinue) {
        ...
        console.log(`ページ ${pageNum} で ${pageReservations.length} 件の予約を取得`);
        for (const reservation of pageReservations) {
            ...
            reservationDate.setHours(parseInt(timeStr.split(":")[0]), parseInt(timeStr.split(":")[1]));
            console.log(`Parsed Reservation date: ${reservationDate}, Last sync time: ${lastSyncTime}`);
...
async function selectDateFromCalendar(page, date) {
    ...
    // 年月が合うまでカレンダーを移動
    while (true) {
        const currentYearMonth = await page.$eval("#calendarArea th.w65", (el) => el.textContent);
        if (currentYearMonth === `${year}`) {
            const currentMonth = await page.$eval("#calendarArea th.mod_left p", (el) => el.textContent);
    ...
    // 日付を選択
    await page.click(`#calendarArea a[href="#${year}${month.toString().padStart(2, "0")}${day
        .toString()
        .padStart(2, "0")}"]`);
...
async function scrapeReservationTable(page) {
    const reservations = [];
...
async function scrapeReservationRow(row) {
    try {
        const dateTime = await row.$eval("td.rsv_date a", (el) => el.textContent?.trim() || "");
        const [date, time] = dateTime.split("\n").map((s) => s.trim());
        const status = await row.$eval("td:nth-child(2)", (el) => el.textContent?.trim() || "");
        const customerName = await row.$eval("td:nth-child(3) p:first-child", (el) => el.textContent?.trim() || "");
        const reservationId = await row.$eval("td:nth-child(3) p:last-child a", (el) => el.textContent?.replace(/[()]/g, "") || "");
        const staff = await row.$eval("td:nth-child(4)", (el) => el.textContent?.trim() || "");
        const reservationRoute = await row.$eval("td:nth-child(5)", (el) => el.textContent?.trim() || "");
        const menu = await row.$eval("td:nth-child(6)", (el) => el.textContent?.trim() || "");
        const usedPoints = await row.$eval("td:nth-child(7)", (el) => el.textContent?.trim() || "");
        const paymentMethod = await row.$eval("td:nth-child(8)", (el) => el.textContent?.trim() || "");
        const amount = await row.$eval("td:nth-child(9)", (el) => el.textContent?.trim() || "");
        return {
            date,
            time,
            status,
...
async function goToNextPage(page, currentPage) {
    ...
    console.log("次のページが見つかりません。最後のページに到達した可能性があります。");
    return false;
}
