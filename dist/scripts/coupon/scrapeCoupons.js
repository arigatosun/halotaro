...
exports.scrapeCoupons = scrapeCoupons;
const crypto_1 = __importDefault(require("crypto"));
async function scrapeCoupons(page) {
    const coupons = [];
    let rowCount = await page.$$eval(".table_list_store tbody tr", (rows) => rows.length);
    ...
    for (let i = 0; i < rowCount; i++) {
        ...
        try {
            ...
            const match = onclickAttribute.match(/doEdit\(event,\s*'[^']*',\s*'([^']*)'/);
            ...
            if (couponId) {
                ...
                rowCount = await page.$$eval(".table_list_store tbody tr", (rows) => rows.length);
            }
        }
        catch (error) {
...
async function scrapeCouponDetails(page, couponId) {
    try {
        await page.waitForSelector("#TagTA_NM_COUPON_NAME_01", {
            state: "visible",
            timeout: 30000,
        });
        const name = await page.$eval("#TagTA_NM_COUPON_NAME_01", (el) => el.value);
        const category = await page.$eval("#TagSL_NM_COUPON_TYPE_CD_01 option:checked", (el) => el.textContent || "");
        const description = await page.$eval("#TagTA_NM_CONTENT_EXPLANATION_01", (el) => el.value);
        const price = await page.$eval("#TagIN_NM_PRICE_01", (el) => el.value);
        const duration = await page.$eval("#TagSL_NM_SEJYUTSU_AIM_TIME_01", (el) => el.value);
        const isReservable = await page.$eval('input[name="frmCouponEditDto.checkedAutoExpiration"]', (el) => el.checked);
        const imageUrl = await page.$eval("#TagImgCouponPoto", (el) => el.src);
        return {
            name,
            category,
            description,
...
function generateDataHash(coupons) {
