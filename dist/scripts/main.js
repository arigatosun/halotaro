...
async function syncReservations(haloTaroUserId, salonboardUserId, password) {
  ...
  try {
    ...
    } = await (0, scrapeReservation_1.scrapeReservations)(
      page,
      startDate,
      endDate,
      lastSyncTime,
    ...
    await (0, save_reservations_1.saveReservations)(
      reservations,
      haloTaroUserId,
      salonboardUserId,
      startDate,
...
async function syncCoupons(haloTaroUserId, salonboardUserId, password) {
  ...
  try {
    ...
    const { coupons, dataHash } = await (0, scrapeCoupons_1.scrapeCoupons)(
      page
    );
    ...
    const processedCoupons = (0, processCouponData_1.processCouponData)(
      coupons,
      haloTaroUserId
    );
    console.log("クーポン情報を処理中...");
    const result = await (0, save_coupons_1.saveCoupons)(
      processedCoupons,
      haloTaroUserId,
      dataHash
    );
...
async function syncReservationToSalonboard(
  ...
  try {
    ...
    const reserveUrl = `https://salonboard.com/KLP/reserve/ext/extReserveRegist/?staffId=${staffId}&date=${(0,
    date_fns_1.format)(startTime, "yyyyMMdd")}&rsvHour=${(0, date_fns_1.format)(
      startTime,
      "HH"
    )}&rsvMinute=00&rlastupdate=20240919182008`;
    await page.goto(reserveUrl);
    // 予約フォームを入力
    await (0, syncReseravtionToSalonboardHelpers_1.fillReservationForm)(
      page,
      reservationData
    );
    console.log("予約を送信中...");
...
async function syncAllData(haloTaroUserId) {
  ...
  if (error || !data) {
    // 状態をエラーに更新
    await supabase.from("salonboard_sync_status").upsert({
      user_id: haloTaroUserId,
      status: "error",
      error_message: "認証情報の取得に失敗しました",
  ...
  try {
    ...
    console.error("全体の同期中にエラーが発生しました:", error);
